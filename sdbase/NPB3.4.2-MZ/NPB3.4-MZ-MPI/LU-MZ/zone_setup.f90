       subroutine zone_setup(nx, nxmax, ny, nz, nx1)
 ...
 98    format(/' Zone sizes:')
       ...
       do j = 1, y_zones
         do i = 1, x_zones
           ...
           nx1(zone_no) = x_size(i)
           ny(zone_no) = y_size(j)
           nz(zone_no) = gz_size

!        nx is the subdomain size partitioned from nx1
           call subdomain(nx1(zone_no), ny(zone_no), nz(zone_no),  &
     &                    nx(zone_no))
           ...
           if (npb_verbose .gt. 1 .and. myid .eq. root) then
             write (*,99) zone_no, nx1(zone_no), ny(zone_no),  &
     &                    nz(zone_no), nx(zone_no)
           endif
         end do
       end do

 99    format(i5,':  ',i5,' x',i5,' x',i5,' (nxp=',i5,')')
       ...
       do iz = 1, proc_num_zones
         ...
         zone_size = (nxmax(zone)+2)*ny(zone)*nz(zone)
         zone2 = (nxmax(zone)+4)*ny(zone)*nz(zone)
         if (iz .eq. 1) then
           start1(iz) = 1
           start5(iz) = 1
         endif
         if (iz .ne. proc_num_zones) then
           ...
           start5(iz+1) = start5(iz) + zone2*5
         else
           tot_zonesize = start1(iz) + zone_size - 1
           tot_zonesize5 = start5(iz) + zone2*5 - 1
         endif
       enddo
...
! ...  for 'qbc_in'
       ...
       do 10 ip = 0, num_procs-1
         ...
         do 15 zone = 1, num_zones
           if (zone_proc_id(zone) .ne. colp) goto 15

           x_face_size = (ny(zone)-2)*(nz(zone)-2)*5
           y_face_size = (nx(zone)-0)*(nz(zone)-2)*5

           zone2 = iz_west(zone)
           ip_east  = zone_proc_id(zone2)*xdim + xdim - 1
           if (ip_east .eq. myid .and. rowp .eq. 0) then
             qstart2_east(zone2) = qoffset
             qoffset = qoffset + x_face_size
           endif

           zone2 = iz_east(zone)
           ip_west  = zone_proc_id(zone2)*xdim
           if (ip_west .eq. myid .and. rowp .eq. xdim-1) then
             qstart2_west(zone2) = qoffset
             qoffset = qoffset + x_face_size
           endif

           zone2 = iz_south(zone)
           ip_north = zone_proc_id(zone2)*xdim + rowp
           if (ip_north .eq. myid) then
             qstart2_north(zone2) = qoffset
             qoffset = qoffset + y_face_size
           endif

           zone2 = iz_north(zone)
           ip_south = zone_proc_id(zone2)*xdim + rowp
           if (ip_south .eq. myid) then
...
! ...  for 'qbc_out'
       ...
       do 20 ip = 0, num_procs-1
         ...
         do 30 zone = 1, num_zones
           if (zone_proc_id(zone) .ne. col) goto 30

           ip_west  = zone_proc_id(iz_west(zone))*xdim + xdim - 1
           ip_east  = zone_proc_id(iz_east(zone))*xdim
           ip_south = zone_proc_id(iz_south(zone))*xdim + row
           ip_north = zone_proc_id(iz_north(zone))*xdim + row

           x_face_size = (ny(zone)-2)*(nz(zone)-2)*5
           y_face_size = (nx(zone)-0)*(nz(zone)-2)*5

           if (ip_west .eq. ip .and. north .eq. -1) then
             qstart_west(zone) = qoffset
             qoffset = qoffset + x_face_size
           endif

           if (ip_east .eq. ip .and. south .eq. -1) then
...
! ...  for intra-process zone copy
       do 40 zone = 1, num_zones
         if (zone_proc_id(zone) .ne. col) goto 40
         ...
         if (ip_west  .eq. myid .and. north .eq. -1) then
           qstart_west(zone)  = qoffset
           qoffset = qoffset + x_face_size
         endif
         if (ip_east  .eq. myid .and. south .eq. -1) then
           qstart_east(zone)  = qoffset
           qoffset = qoffset + x_face_size
         endif
         if (ip_south .eq. myid) then
...
! ...  set up cyclic communication group
       iz = 1
       do while (iz .lt. myid)
         iz = iz * 2
       enddo
