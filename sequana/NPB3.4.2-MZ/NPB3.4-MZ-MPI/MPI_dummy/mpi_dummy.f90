...
! may be valid to call this in single processor case
      ...
      subroutine mpi_init_thread(requested, provided, ierr)
      implicit none
      include 'mpif.h'
      integer requested, provided, ierr
...
! assume double precision, which is all SP uses 
     ...
     &                      dtype, op, root, comm, ierr)
      ...
      if (loc(inbuf) .eq. loc(mpi_in_place)) return
     ...
     &                        outbuf, nitems, dtype, comm, ierr)
      ...
      subroutine mpi_allgather(inbuf, incnt, intype,  &
     &                      outbuf, oucnt, outype,  &
     &                      comm, ierr)
      implicit none
      integer incnt, intype, oucnt, outype, comm, ierr
      integer inbuf(*), outbuf(*)
      call mpi_alltoall(inbuf, incnt, intype, outbuf, oucnt,  &
     &                        outype, comm, ierr)
